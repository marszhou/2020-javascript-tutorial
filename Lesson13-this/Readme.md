...
const person = {
  ...
  loop2: function (count) {
    // wrong
    ;[...Array(count)].forEach(function () {
      this.describe()
    })
  },
  loop3: function (count) {
    // correct
    const self = this
    ;[...Array(count)].forEach(function () {
      self.describe()
...
_第一个参数和 bind 一样，是指函数运行上下文所需的 this 是哪个，这里不演示了_
